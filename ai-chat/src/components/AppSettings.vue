...
<template>
  <fwb-modal v-if="settingsStore.areSettingsVisible" @close="settingsStore.hideSettings()" not-escapable persistent>
    ...
    <template #body>
      <div class="mb-3"><fwb-input v-model="form.openaiApiKey" label="Andrual API key" type="password" /></div>
      <div class="mb-3"><fwb-select v-model="form.openaiModel" label="Model" placeholder="Select one" :options="models" /></div>
      <div class="mb-3"><fwb-input v-model="form.openaiMaxTokens" label="Max tokens" type="number" hidden /></div>
      <div><fwb-input v-model="form.openaiTemp" label="Temperature" type="number" step="0.1"  hidden /></div>
    </template>
    <template #footer>
